function [ result, class_val] = tr_train( base, config, info )

    switch (config.classification.svmType)
        case {'0','1','2'}
            [ result, class_val] = tr_svm_train( base, config );
        case {'3','4'}
            [ result, class_val] = tr_svr_train( base, config );
        otherwise
            [ result, class_val] = tr_svm_train( base, config );
    end

function [ result, class_val] = tr_svm_train( base, config, info )
...
%     	[~, ~, predict, command] = lib_svm.find_cost_gamma(class_obs, class_val, 'autoweight',config.classification.autoweight);
        [~, ~, predict, command] = lib_svm.find_cost_gamma(class_obs, class_val, 'autoweight',config.classification.autoweight,...
                                                                                 ...
                                                                                 'parpool_enable',config.classification.parpoolEnable);   
...
%         ratemodel = lib_svm.train(class_obs, class_val, [result.command, ' -v 10', ' -s ',config.classification.svmType, ' -t ',config.classification.kernelType ]); % for SVM
...
function [ result, class_val] = tr_svr_train( base, config, info )
